  ...
  save() {
    ...
    if (this.formFruta.valid) {
      ...
      if (this.accion == "add") {
        ...
        let urlImg: string[]  = datos_form.imagen?.split("\\") ?? [];
        //format para la imagen
        let ultimoElement: string = urlImg[urlImg.length-1];
        ultimoElement = `../../../assets/${ultimoElement}`;
        let nuevo: Fruta = {
          codigo: _nuevo_id,
          denominacion: datos_form.denominacion as string,
          imagen: ultimoElement as string,
        ...
        this.fruta.denominacion = datos_form.denominacion || this.fruta.denominacion;
